...
# 1. 定义临时BAT文件路径（修复空格问题，避免cmd执行报错）
...
$TargetAcadDir = "D:\Autodesk\AutoCAD 2026"                      # 目标安装目录
$TargetAcadExe = Join-Path $TargetAcadDir "acad.exe"             # 目标替换文件路径

# 确保解压目标目录存在（新增：避免解压时目录不存在报错）
...
if (-not $skipAll) {
    ...
    try {
        ...
        # 找到压缩包内的顶层文件夹名（如 AutoCAD_2026_Shell_YJ\）
        ...
        foreach ($entry in $entries) {
            if (-not [string]::IsNullOrEmpty($entry.Name)) {
                # 核心修复：截掉顶层文件夹路径，直接解压到FinalDir根目录

...
# 5. 核心配置（适配2023版：Img→image、666.lnk→Install AutoCAD 2023.bat）
...
$SetupBatPath = Join-Path $FinalDir "Install AutoCAD 2023.bat" # 替换666.lnk为指定bat文件
...
if (-not (Test-Path $SetupBatPath)) {
    Write-Host "❌ 未找到安装文件 Install AutoCAD 2023.bat" -ForegroundColor Red
    Read-Host "按任意键退出"
    exit 1
}
